...
public class IFDSUninitializedVariablesPDFsam {

	private final static int TEST_COUNT = 10;

	private abstract class DynamicTestHandler implements ITestHandler<Local> {
...
//				SootClass scNew = Scene.v().forceResolve(sc.getName(), SootClass.BODIES);
	 ...
	 */
	private void performTestDirect(final ITestHandler<Local> handler, final String className) {
		soot.G.reset();
		handler.initialize();
...
//				"-exclude", "javax",
	 ...
	 * @param className The name of the test class to use
	 */
	private void performTestRerun(final ITestHandler<Local> handler, final String className) {
		soot.G.reset();
 		handler.initialize();
			...
			protected void internalTransform(String phaseName, @SuppressWarnings("rawtypes") Map options) {
				...
				if (handler != null) {
					...
					for (int i = 0; i < handler.getPhaseCount(); i++) {
						...
						IFDSTabulationProblem<Unit, Local, SootMethod, InterproceduralCFG<Unit, SootMethod>> problem2 =
							new IFDSUninitializedVariables(icfg = new JimpleBasedInterproceduralCFG());
						IFDSSolver<Unit,Local,SootMethod,InterproceduralCFG<Unit,SootMethod>> solver2 =
							new IFDSSolver<Unit,Local,SootMethod,InterproceduralCFG<Unit,SootMethod>>(problem2);
...
//				"-exclude", "javax",
	...
	private ITestHandler<Local> ITestHandlerNewVersion() {
